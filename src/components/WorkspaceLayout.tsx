...
export default function WorkspaceLayout() {
  ...
  const [isCreateBrewOpen, setIsCreateBrewOpen] = useState(false);
  const [newBrewName, setNewBrewName] = useState("");
  const [newBrewDescription, setNewBrewDescription] = useState("");
  ...
  const handleCreateBrew = async (e: React.FormEvent) => {
    e.preventDefault();

    const name = newBrewName.trim();
    if (!name) return;

    const brew: BrewProject = {
      id: crypto.randomUUID(),
      name,
      description: newBrewDescription.trim() || "A fresh project brew.",
      starterPrompts: [],
      contextFiles: [],
      createdAt: Date.now(),
    };

    await dbHelpers.saveBrew(brew);
    await loadWorkspaceData();
    setActiveBrewId(brew.id);
    setActiveSessionId(null);
    setViewMode("project");
    setIsCreateBrewOpen(false);
    setNewBrewName("");
    setNewBrewDescription("");
  };
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-6 pr-2">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between pl-2 mb-2">
                <h3 className="text-xs font-bold text-coffee-500 dark:text-coffee-400 uppercase tracking-wider">
                  Your Brews
                </h3>
                <button
                  type="button"
                  onClick={() => setIsCreateBrewOpen((open) => !open)}
                  className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider bg-coffee-700 text-white hover:bg-coffee-800 dark:bg-coffee-600 dark:hover:bg-coffee-700 transition"
                >
                  <Plus size={11} />
                  New Brew
                </button>
              ...
              {isCreateBrewOpen && (
                <form
                  ...
                  className={`space-y-2 rounded-2xl p-3 border ${isContrast ? "bg-zinc-950 border-zinc-800" : "bg-white/70 dark:bg-coffee-950/70 border-coffee-300/40 dark:border-coffee-800/40"}`}
                >
                  <input
                    value={newBrewName}
                    onChange={(e) => setNewBrewName(e.target.value)}
                    placeholder="Project name"
                    className={`w-full rounded-xl px-3 py-2 text-sm focus:outline-none border ${isContrast ? "bg-black border-zinc-800 text-zinc-100 placeholder-zinc-500" : "bg-white/80 dark:bg-coffee-900/80 border-coffee-300/40 dark:border-coffee-800/40 text-coffee-900 dark:text-coffee-100 placeholder-coffee-400"}`}
                  />
                  <textarea
                    value={newBrewDescription}
                    onChange={(e) => setNewBrewDescription(e.target.value)}
                    placeholder="Short project description"
                    rows={3}
                    className={`w-full rounded-xl px-3 py-2 text-sm resize-none focus:outline-none border ${isContrast ? "bg-black border-zinc-800 text-zinc-100 placeholder-zinc-500" : "bg-white/80 dark:bg-coffee-900/80 border-coffee-300/40 dark:border-coffee-800/40 text-coffee-900 dark:text-coffee-100 placeholder-coffee-400"}`}
                  />
                  <div className="flex items-center gap-2">
                    <button
                      type="submit"
                      disabled={!newBrewName.trim()}
                      className="flex-1 rounded-xl px-3 py-2 text-sm font-medium bg-coffee-700 text-white hover:bg-coffee-800 disabled:opacity-40 transition dark:bg-coffee-600 dark:hover:bg-coffee-700"
                    >
                      Create Brew
                    </button>
                    <button
                      ...
                      className={`rounded-xl px-3 py-2 text-sm font-medium border transition ${isContrast ? "bg-black border-zinc-800 text-zinc-200 hover:bg-zinc-900" : "bg-white/70 dark:bg-coffee-950/70 border-coffee-300/40 dark:border-coffee-800/40 text-coffee-700 dark:text-coffee-200 hover:bg-coffee-100 dark:hover:bg-coffee-900/50"}`}
                    >
                      Cancel
                    </button>
                  </div>
